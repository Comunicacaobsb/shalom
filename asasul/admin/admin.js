/* =====================================================================
   Painel Shalom Asa Sul — lógica (Supabase Auth + edição)
   ===================================================================== */
(function () {
  "use strict";
  ...
  function showApp(email){ loginView.classList.add("hidden"); appView.classList.remove("hidden"); $("#userEmail").textContent=email||""; loadEvents(); loadSchedules(); }
  ...
  function loadSchedules(){
    if (!sb) return;
    sb.from("settings").select("value").eq("key","schedules").limit(1).then(function(res){
      var val = (res.data && res.data[0] && res.data[0].value) || {};
      CATS.forEach(function(c){
        var ta=$("#sch-"+c.key); var arr=val[c.key]||[];
  ...
  $("#saveSchedBtn").addEventListener("click", function(){
    ...
    var value={};
    CATS.forEach(function(c){
      value[c.key] = $("#sch-"+c.key).value.split("\n").map(function(l){return l.trim();}).filter(Boolean);
    });
    btn.disabled=true; setMsg(msg,"Salvando…",true);
    sb.from("settings").upsert({ key:"schedules", value:value, updated_at:new Date().toISOString() }, { onConflict:"key" })
      .then(function(res){
        btn.disabled=false;
  ...
  function loadEvents(){
    ...
    sb.from("events").select("*").order("position",{ascending:true}).then(function(res){
  ...
  function renderList(){
    if (!currentEvents.length){ listEl.innerHTML='<p class="center" style="color:var(--ink-faint)">Nenhum evento ainda. Clique em "Novo evento".</p>'; return; }
  ...
  // slug automático a partir do título (só se o slug estiver vazio)
  ...
  $("#modalSave").addEventListener("click", function(){
    ...
    op.then(function(res){
      btn.disabled=false;
      if (res.error){
        if (res.error.code==="23505") setMsg(msg,"Já existe um evento com esse slug. Escolha outro.");
